  ...
  fetch(apiURL)
    ...
    .then((data) => {
      ...
      data.forEach((item) => {
        ...
        listItemContainer.classList.add("d-flex", "align-items-center", "justify-content-center", "w-100", "mx-2", "py-2", "col-lg-7");
        const listItem = document.createElement("li");
        listItem.textContent = `${item.title} - €${item.price}`;
        const addButton = document.createElement("button");
        addButton.textContent = "Add";
        // Pass the data directly to the click handler
        addButton.addEventListener("click", () => handleSearchListItemClick(item));
